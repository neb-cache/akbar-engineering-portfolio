...
export function ResumeLink({ url, inverse = false }: { url: string | null; inverse?: boolean }) {
  ...
  return <a href={url} target="_blank" rel="noreferrer" onClick={() => trackPublicEvent("Resume downloaded")} aria-label="Download Akbar A.R resume (opens in a new tab)" className={`button-base ${classes}`}><FileDown size={15}/> Download resume</a>;
}
