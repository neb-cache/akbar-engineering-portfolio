...
export default async function AboutPage() {
  ...
  return <><div className="public-container py-16 sm:py-24"><SectionHeading label="About the engineer" title="Systems thinking, practical delivery, accountable ownership."/><section className="paper-panel grid gap-8 p-7 sm:p-12 lg:grid-cols-[.7fr_1.3fr]"><div><p className="editorial-label !text-[var(--accent-brown)]">Professional overview</p><p className="mt-7 font-serif text-4xl leading-[1.05]">Engineer and technical leader building systems, people, and operational capability.</p></div><p className="text-base leading-8 text-[rgba(24,19,15,.72)]">I am a full-stack and systems engineer focused on connecting software, business processes, infrastructure, and people. My work spans Golang services, Next.js platforms, Flutter applications, ERP integration, automation, and production operations.</p></section><div className="mt-16 divide-y divide-[var(--border)] border-y border-[var(--border)]">{records.map(([title,content],index)=><section key={title} className="grid gap-5 py-9 md:grid-cols-[4rem_15rem_1fr]"><span className="font-mono text-xs text-[var(--accent-gold)]">{String(index+1).padStart(2,"0")}</span><h2 className="font-serif text-3xl">{title}</h2><p className="max-w-3xl text-sm leading-7 text-[var(--text-secondary)]">{content}</p></section>)}</div><section className="mt-16"><p className="editorial-label">Technology range</p><div className="mt-6 flex flex-wrap gap-x-5 gap-y-3 border-y border-[var(--border)] py-6 font-mono text-[.68rem] uppercase tracking-[.08em] text-[var(--paper-soft)]">{technologies.map((item)=><span key={item}>{item}</span>)}</div></section></div><BuilderOfSystemsPeople profile={profile} records={mentorship}/><ContactCta profile={profile}/></>;
}
