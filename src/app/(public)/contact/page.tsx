    ...
    <div className="public-container py-16 sm:py-24">
      ...
      <div className="grid min-w-0 gap-12 lg:grid-cols-[.7fr_1.3fr]">
        <aside className="min-w-0 border-t border-[var(--accent-gold)] pt-6">
          ...
          <div className="mt-8 space-y-3"><p className="flex items-center gap-3 text-sm"><MapPin aria-hidden="true" size={16} className="shrink-0 text-[var(--accent-gold)]" />{profile.location}</p>{links.map(({ label, href, icon: Icon }) => <a key={href} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined} className="button-base button-editorial !flex !w-fit max-w-full !justify-start !normal-case !tracking-normal"><Icon aria-hidden="true" size={16} className="shrink-0 text-[var(--accent-gold)]" /><span className="truncate">{label}</span></a>)}</div>
