  ...
  return (
    <>
      <div className="public-container py-16 sm:py-24">
        <SectionHeading label="Professional record" title="From full-stack delivery to systems ownership." description="A chronological record spanning application delivery, enterprise integration, architecture, infrastructure coordination, people development, and production escalation." />
        <div className="mb-14 grid gap-px bg-[var(--border)] sm:grid-cols-2 lg:grid-cols-4">{scopes.map(([title, copy], index) => <article key={title} className="interactive-panel bg-[var(--surface)] p-6"><span className="font-mono text-xs text-[var(--accent-gold)]">{String(index + 1).padStart(2, "0")}</span><h2 className="mt-5 break-words font-serif text-2xl">{title}</h2><p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{copy}</p></article>)}</div>
        <section className="mb-16 border-y border-[var(--border)] py-7"><p className="editorial-label">Career progression / PT Sinergia Beaute Indonesia</p><p className="mt-4 max-w-4xl break-words font-serif text-[clamp(2rem,6vw,2.75rem)] leading-tight">Full-Stack Engineer — ERP Integration & Business Systems <span className="text-[var(--accent-gold)]">→</span> Principal Full-Stack & Systems Engineer</p><p className="reading-measure mt-4 text-sm leading-7 text-[var(--text-secondary)]">Expanded from ERP integration and application delivery into architecture ownership, infrastructure supervision, multi-platform coordination, and production escalation.</p></section>
